...
parse_file = function(file, configuration=list(traversal_order = "pre-order", prunning_level=0.1, remove_aggregates=F, remove_named_empty_cols=F, interpolate_spanning_column_header_cells=T, interpolate_spanning_column_data_cells=F, conservative_type_casting=T, separate_multiple_units=T, only_one_table=T)){
  results_and_tree = generate_parsing_tree(file,configuration)
  quality_ranking = rank_quality(results_and_tree$results)
